# Мониторинга статуса товаров на сайте Eltex-co.ru

## Описание Workflow

**Eltex State Workflow** — автоматизация мониторинга статуса товаров на сайте eltex-co.ru для дилеров. Получает данные из API Eltex, сравнивает статусы товаров, логирует изменения в Supabase, уведомляет в Telegram при изменениях. Триггеры: Manual и Schedule (09:20).


## В этом рабочем процессе я показываю часть работы по глобальному отслеживанию изменений на сайте eltex-co. 
В ежедневном рабочем процессе участуют **несколько проверок**:
- на количество товаров в каталоге,
- на ответ страниц кодом 404,
- на изменение состава datdscheet,
- **на изменение статуса тоавров каталога** (**ЭТОТ WORKFLOW**!),
- на изменение количества товаров в разделах сайта.
Для официального дилера крайне важно отслеживать **ряд маркеров**, чтобы своевременно менять информацию на своем сайте. При помощи этого процесса сотрудники официального дилера **сэкономили около 20 рабочих часов в месяц**. Теперь не нужно вручную проверять данные - бот оповестит утром, если есть изменения у завода-производителя!
